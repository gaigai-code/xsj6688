import { deleteThemeAsset, getThemeAssetDataUrl, saveThemeAssetFromBlob } from "./theme-storage";
...
// 删除聊天资产（角色参考图、聊天背景等复用同一套 IndexedDB 资产存储）。
export async function deleteChatImageFromIndexedDB(id: string): Promise<void> {
    if (!id) return;
