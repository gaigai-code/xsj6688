...
每条 prompt 有一组标签组合（tagCombos，多选）。每个组合是一组 tags（例如 ["chat", "text"]），第一个是主场景，后面是子细化。
命中任一组合即生效；没有标签（空数组）的条目在所有场景注入（视为"全局"）。
...
注意：
· 标签现在是「多组合」字段 tagCombos。「读取预设」结果里每条 prompt 会以 tags=[chat,text] [chat,voice] 的形式显示它命中的所有组合（旧的 featureTag/tags 字段系统会自动兼容归一）。
· 新增条目：用「添加预设条目」时可直接传 tagCombos（多选，例 [["chat","text"],["chat","voice"]]）。改已有条目的标签：用「更新预设标签」，传 [] 清空成「通用」。改 content / name 等内容字段：用「更新预设条目」。
· 标签名必须用「读取预设」里显示的原始值，不要臆造。
